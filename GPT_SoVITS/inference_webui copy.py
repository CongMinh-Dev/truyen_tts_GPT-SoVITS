...
def get_bert_inf(phones, word2ph, norm_text, language):
    ...
    if language == "zh":
        print(">>> DEBUG: Đang chạy BERT cho Tiếng Trung"); sys.stdout.flush()
        bert = get_bert_feature(norm_text, word2ph).to(device)
    elif language == "vi":
        print(">>> DEBUG: Đang chạy BERT (PhoBERT-large) cho Tiếng Việt"); sys.stdout.flush()
        # Nếu hàm get_bert_feature của bạn dùng chung cho cả vi và zh:
        bert = get_bert_feature(norm_text, word2ph).to(device)
    else:
        print(f">>> DEBUG: Tạo tensor 0 cho ngôn ngữ {language}"); sys.stdout.flush()
        bert = torch.zeros((1024, len(phones)), 
                           dtype=torch.float16 if is_half else torch.float32).to(device)
    return bert
...
def get_phones_and_bert(text, language, version, final=False):
    ...
    for i in range(len(textlist)):
        ...
        print(f">>> DEBUG: Bắt đầu xử lý Cleaner cho ngôn ngữ: {lang}"); sys.stdout.flush()
        phones, word2ph, norm_text = clean_text_inf(textlist[i], lang, version)
        print(f">>> DEBUG: Đã xong Cleaner. Số lượng phones: {len(phones)}"); sys.stdout.flush()

        print(f">>> DEBUG: Bắt đầu gọi hàm get_bert_inf..."); sys.stdout.flush()
        bert = get_bert_inf(phones, word2ph, norm_text, lang)
        print(f">>> DEBUG: Đã nhận được tensor BERT. Kích thước: {bert.shape}"); sys.stdout.flush()
